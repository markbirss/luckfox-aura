...
msg_info "${FUNCNAME[0]}-$LINENO : its file       = $its_file"
msg_info "${FUNCNAME[0]}-$LINENO : ramdisk file   = $ramdisk_file"
msg_info "${FUNCNAME[0]}-$LINENO : kernel's image = $kernel_image"
msg_info "${FUNCNAME[0]}-$LINENO : dtb file       = $dtb_file"
msg_info "${FUNCNAME[0]}-$LINENO : resource image = $resource_image"
msg_info "${FUNCNAME[0]}-$LINENO : target file    = $target_file"
msg_info "${FUNCNAME[0]}-$LINENO : target arch    = $target_arch"
msg_info "${FUNCNAME[0]}-$LINENO : optional param = $optional_param"
msg_info "${FUNCNAME[0]}-$LINENO : ramdisk_r      = $ramdisk_r"
msg_info "${FUNCNAME[0]}-$LINENO : ramdisk_r_size = $ramdisk_r_size"
msg_info "${FUNCNAME[0]}-$LINENO : ramdisk_c      = $ramdisk_c"
msg_info "${FUNCNAME[0]}-$LINENO : ramdisk_c_size = $ramdisk_c_size"
msg_info "${FUNCNAME[0]}-$LINENO : kernel_c       = $kernel_c"
...
do
	...
	if [ $kernel -eq 2 ];then
		if echo $line | grep -w "^compression"; then
			kernel=3
			if echo $optional_param | grep -w "kernel_compression_none" ; then
				echo "compression = \"none\";" >> $target_its_file
				continue
			fi
		fi
	fi
	...
	if [ -f $ramdisk_file ]; then
		...
		if [ $ramdisk -eq 2 ];then
			if echo $line | grep -w "^compression"; then
				ramdisk=3
				if echo $optional_param | grep -w "ramdisk_compression_xz" ; then
					echo "compression = \"xz\";" >> $target_its_file
					continue
				fi
				if echo $optional_param | grep -w "ramdisk_compression_gzip" ; then
					echo "compression = \"gzip\";" >> $target_its_file
					continue
				fi
			fi
		fi
		...
		if echo $line | grep -w "^decomp-async" ; then
			if echo $optional_param | grep -w "decomp-async_none" ; then
				msg_info "found decomp-async_none flag, skip config decomp-async for ramdisk."
				continue
			fi
		fi
	fi
...
if [ "$target_arch" = "arm64" ]; then
    sed -i 's/arch.*=.*\"arm\"\;/arch = \"arm64\"\;/g' $target_its_file
fi
