...
function mkufs(){
	...
	fileSectorCount=$2
	dstname=$3
	echo " scsi write \${ramdisk_addr_r} $offset $fileSectorCount;" >> $dstname
}
...
function mkcmdline(){
	...
	elif [ "$BOOT_MEDIUM" == "ufs" ];then
		mkufs $partitionOffsetSector $fileSizeSector $TFTP_FILE_NAME
		mkufs $partitionOffsetSector $fileSizeSector $SD_FILE_NAME
