...
rkdbg=${rkdbg:-0}
if [ $rkdbg -gt 1 ]; then echo "$0: rkdbg: $rkdbg"; set -x ; fi
if [ $rkdbg -gt 3 ]; then echo "$0: rkdbg: $rkdbg, Enter terminal for debug"; exit 0 ; fi
...
if [ ! -f "$input_file" ];then
	input_file="/userdata/update_ota.tar"

	timeout_count=0
	while [ $timeout_count -lt 10 ]; do
		if grep -q "userdata" /proc/mounts; then
			break
		fi
		sleep .5
		echo "Waiting And Checking for flash device..."
