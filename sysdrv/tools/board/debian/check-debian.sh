...
RK_DEBIAN_ARCH="${ARCH:-armhf}"
RK_DEBIAN_VERSION="${RK_DEBIAN_VERSION:-trixie}"
...
if [ ! -e "/usr/share/live/build/data/debian-cd/$RK_DEBIAN_VERSION" ]; then
	...
	echo "git clone https://salsa.debian.org/live-team/live-build.git --depth 1 -b debian/1%20250814"
	echo "cd live-build"
...
if [ ! -e "/usr/share/debootstrap/scripts/$RK_DEBIAN_VERSION" ]; then
	...
	echo "git clone https://salsa.debian.org/installer-team/debootstrap.git --depth 1 -b 1.0.142"
	echo "cd debootstrap"
	echo "sudo make install -j8"
	echo -e "\e[0m"
	exit 1
