...
if [ ! $DISPLAY_SYSTEM ]; then
	DISPLAY_SYSTEM="none"
fi

echo -e "\033[36m Building for $VERSION \033[0m"

if [ ! -e linaro-trixie-alip-*.tar.gz ]; then
	echo "\033[36m Run mk-base-debian.sh first \033[0m"
	exit -1
fi

echo -e "\033[36m Extract image \033[0m"
sudo tar -xpf linaro-trixie-alip-*.tar.gz
...
echo "deb http://mirrors.ustc.edu.cn/debian/ trixie-backports main contrib" >> /etc/apt/sources.list
echo "deb-src http://mirrors.ustc.edu.cn/debian/ trixie-backports main contrib" >> /etc/apt/sources.list
...
if [ "$DISPLAY_SYSTEM" == "enable" ]; then
#---------Camera---------
echo -e "\033[36m Install desktop camera.................... \033[0m"
\${APT_INSTALL} cheese
...
useradd -m -s /bin/bash aura && echo "aura:luckfox" | chpasswd && usermod -aG sudo aura
echo 'export PATH=\$PATH:/sbin:/usr/sbin:/oem/usr/bin:/oem/usr/sbin' >> /etc/profile
update-locale LANG=POSIX

mkdir /oem
mkdir /userdata
mkdir /data

if [ -f '/usr/lib/systemd/system/initd-seq.service' ]; then
	systemctl enable initd-seq
fi

if [ -f '/usr/lib/systemd/system/check-ssh-on-login.service' ]; then
	systemctl enable check-ssh-on-login.service
fi

if [ -f '/usr/lib/systemd/system/ntpdate.service' ]; then
