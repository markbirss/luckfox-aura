...
if [ "$block_size_4K" = "YES" ]; then
__mkfs_ext4_opt="-b 4096"
fi

mkdir -p $(dirname $dst)

echo MKE2FS_CONFIG=$cwd/mke2fs.conf mkfs.ext4 -d $src -r 1 -N 0 -m 5 -L \"\" -O ^64bit,^huge_file $dst \"$dst_size\" $__mkfs_ext4_opt
MKE2FS_CONFIG=$cwd/mke2fs.conf mkfs.ext4 -d $src -r 1 -N 0 -m 5 -L "" -O ^64bit,^huge_file $dst "$dst_size" $__mkfs_ext4_opt
if [ $? != 0 ]; then
