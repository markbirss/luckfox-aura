...
function help_msg()
{
	...
	echo -e "$(basename $0) <source dir> <output dir> <partition size> <partition name> <fs type> <fs comp> <optional flag>"
	echo -e ""
	...
	echo -e ""
	echo -e "<optional flag>: optional flag"
	echo -e "           NONE:  add ubi layout for target image"
	exit 0
}
...
function mk_ubi_image_fake()
{
	...
	if [ "$UBI_OPTIONAL_FLAG" = "NONE" ];then
		UBIFS_TYPE=NONE
	fi

	case $UBIFS_TYPE in
		NONE)
			msg_info "Ignore Generate image"
			temp_image=$ROOTFS_SRC_DIR/${UBI_VOL_NAME}.img
			;;
		squashfs)
			...
			if [ "$GENERATE_STATIC_IMAGE_DONE" = "NO" ];then
	...
	if [ -n "$RK_UBI_HOOK_IMAGE_DIR" ];then
		echo "mkdir -p $RK_UBI_HOOK_IMAGE_DIR" >> $UBI_IMAGE_FAKEROOT
		echo "cp -fa $temp_image $RK_UBI_HOOK_IMAGE_DIR/${UBI_VOL_NAME}.img" >> $UBI_IMAGE_FAKEROOT
	fi
...
GENERATE_STATIC_IMAGE_DONE=YES
